...
if __name__ == "__main__":
    config = {
        ...
        "ACTIVATION": "relu",
        "ENV_NAME": "TabularMDP",
        "ENV_FILE": "/nas/ucb/cassidy/rl-theory/data/mdps/assault_10_fs30/consolidated.npz",  # Path to your .npz file
        "REWARD_SCALE": 1/21,
        "EVAL_FREQUENCY": 1000,          # Evaluate every 1000 env steps
        "TRAIN_MEDIAN_WINDOW": 20,       # Window size for median training reward
        "OPTIMAL_REWARD": 6.0,          # Optimal reward threshold (after scaling)
    }
    steps_per_update = config["NUM_ENVS"] * config["NUM_STEPS"]
