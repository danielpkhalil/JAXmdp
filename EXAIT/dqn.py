...
def run_dqn(config):
    # initialize W&B
    wandb.init(project="DQN", config=config, mode=config.get("WANDB_MODE", "disabled"))
    ...
    # run training loop
    rng = jax.random.PRNGKey(config["SEED"])
    rngs = jax.random.split(rng, config.get("NUM_SEEDS", 1))
    ...
    for seed_idx, seed_rng in enumerate(rngs):
        ...
        for u in range(config["NUM_UPDATES"]):
            ...
            # log
            writer.writerow({
                "seed": seed_idx, "update": u, "timesteps": t,
            ...
            if max(best_eval, best_med_train) >= config["OPTIMAL_REWARD"]:
                print(f"[Seed {seed_idx}] reached optimal reward at timesteps {t}")
                break
