...
Combined PPO script that:
 - Uses per-update chunking (hybrid style)
 - vmaps over multiple seeds
 - Performs intermittent evaluation
 - Logs mean training return, eval return, best eval, etc.
 - Optionally logs online progress (for one seed) via wandb
 - At the end, plots the reward curve and logs it to wandb

Caveats:
 - Because of vmap, in-loop logging is only supported via a separate single-seed run.
"""
...
class MiniGridCNNActorCritic(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Conv(features=32, kernel_size=(3, 3), strides=(2, 2),
                    padding="SAME", kernel_init=orthogonal(jnp.sqrt(2)))(x)
        x = nn.relu(x)
...
def make_rollout_and_update_fn(env, env_params, network, config):
    ...
    def rollout_and_update(train_state, env_state, last_obs, rng):
        def env_step(carry, _):
            ts, es, obs, rng_ = carry
            rng_, rng_act = jax.random.split(rng_)
            pi, val = network.apply(ts.params, obs)
            ...
            transition = Transition(done=done, action=action, value=val,
                                    reward=rew, log_prob=logp, obs=obs, info=info)
        ...
        def ppo_update(ts, traj_, adv_, ret_, rng_):
            ...
            def update_mb(ts, mb):
                ...
                def loss_fn(params, t, adv, ret):
                    ...
                    v_loss = 0.5 * jnp.mean(jnp.maximum((val - ret)**2, (v_clipped - ret)**2))
                    ratio = jnp.exp(logp - t.log_prob)
                    adv_norm = (adv - adv.mean()) / (adv.std() + 1e-8)
                    p_loss = -jnp.mean(jnp.minimum(ratio * adv_norm,
                ...
                (loss_val, aux), grads = grad_fn(ts.params, mb_traj, mb_adv, mb_ret)
            ...
            def scan_mb(ts, _):
                def one_mb(ts, i):
                    ...
                    ts, _ = update_mb(ts, mb)
                    return ts, None
                idxs = jnp.arange(config["NUM_MINIBATCHES"])
                ts, _ = jax.lax.scan(one_mb, ts, idxs)
                return ts, None
...
# -----------------------------------------------------------------------------
# Deterministic Evaluation (while_loop)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Single-Seed Training (functional, no in-loop logging)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# (Optional) Single-Seed Training with Online Logging
# -----------------------------------------------------------------------------
def train_single_seed_with_logging(rng: jax.random.PRNGKey, config: dict, log_freq: int = 100) -> Dict[str, jnp.ndarray]:
    # This version runs sequentially so that wandb.log can be called inside.
    # (Implementation similar to train_single_seed, with periodic plotting.)
    if config["ENV_NAME"] == "TabularMDP" and TabularEnv is not None:
        env_, env_params_ = TabularEnv(config["ENV_FILE"]), None
        env_params_ = env_.default_params().replace(reward_scale=config["REWARD_SCALE"])
    else:
    ...
    for update_i in range(config["NUM_UPDATES"]):
        ...
        mean_ret = jnp.mean(traj.info["returned_episode_returns"])
        mean_returns_list.append(mean_ret)
        if update_i % log_freq == 0:
            current_curve = jnp.stack(mean_returns_list)
            plt.figure()
            plt.plot(np.array(current_curve), label="Mean Train Return")
            ...
            wandb.log({"online_progress_plot": wandb.Image(img)}, step=update_i)
            plt.close()
    mean_returns_jnp = jnp.stack(mean_returns_list, axis=0)
    return {"mean_train_returns": mean_returns_jnp}
...
if __name__ == "__main__":
    ...
    # Option 1: Run single-seed training with online logging (if desired)
    if config.get("ONLINE_LOGGING", False):
        rng_single = jax.random.PRNGKey(config["SEED"])
        _ = train_single_seed_with_logging(rng_single, config, log_freq=100)

    # Option 2: Run multi-seed training via vmap (final metrics & plots)
    num_seeds = 4
    ...
    # Plot the reward curve for each seed and log to wandb
    ...
    wandb.finish()
    print("Multi-seed training complete and progress plots logged to wandb!")
