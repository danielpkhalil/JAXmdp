...
#         "ENV_FILE": "/nas/ucb/cassidy/rl-theory/data/mdps/MiniGrid-MultiRoom-N2-S4-v0/consolidated.npz",  # Not used unless you do "TabularMDP"
#         "REWARD_SCALE": 1.0,
#         "EVAL_FREQUENCY": 1000,
...
class MiniGridCNNActorCritic(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        # Print input shape immediately upon entry:
        jax.debug.print("MiniGridCNNActorCritic: Input shape = {}", x.shape)
        # Convolution layers
        x = nn.Conv(features=32, kernel_size=(3, 3), strides=(2, 2), padding="SAME")(x)
        jax.debug.print("After first conv: shape = {}", x.shape)
        x = nn.relu(x)
        x = nn.Conv(features=64, kernel_size=(3, 3), strides=(2, 2), padding="SAME")(x)
        jax.debug.print("After second conv: shape = {}", x.shape)
        x = nn.relu(x)
        x = nn.Conv(features=64, kernel_size=(3, 3), strides=(1, 1), padding="SAME")(x)
        jax.debug.print("After third conv: shape = {}", x.shape)
        x = nn.relu(x)

        # Flatten
        x = x.reshape((x.shape[0], -1))
        jax.debug.print("After flatten: shape = {}", x.shape)

        # 512-dim FC
        x = nn.Dense(features=512)(x)
        jax.debug.print("After Dense(512): shape = {}", x.shape)
        x = nn.relu(x)

        # Policy head
        logits = nn.Dense(self.action_dim)(x)
        jax.debug.print("Policy logits shape: {}", logits.shape)
        pi = distrax.Categorical(logits=logits)

        # Value head
        value = nn.Dense(1)(x)
        jax.debug.print("Value output shape: {}", value.shape)
        return pi, jnp.squeeze(value, axis=-1)
...
class MLPActorCritic(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        jax.debug.print("MLPActorCritic: Input shape = {}", x.shape)
        act_fn = nn.relu if self.activation == "relu" else nn.tanh
        x = x.reshape((x.shape[0], -1))
        x = nn.Dense(64, kernel_init=orthogonal(np.sqrt(2)))(x)
        x = act_fn(x)
...
def make_rollout_and_update_fn(env, env_params, network, config):
    @jax.jit
    def rollout_and_update(train_state, env_state, last_obs, rng):
        jax.debug.print("Rollout start: last_obs shape = {}", last_obs.shape)
        # 1) Collect rollout
        def env_step_fn(carry, _):
            train_state, env_state, last_obs, rng = carry
            rng, act_rng = jax.random.split(rng)
            pi, value = network.apply(train_state.params, last_obs)
            jax.debug.print("env_step_fn: last_obs shape = {}", last_obs.shape)
            action = pi.sample(seed=act_rng)
            log_prob = pi.log_prob(action)
            rng, step_rng = jax.random.split(rng)
            step_rngs = jax.random.split(step_rng, config["NUM_ENVS"])
        ...
        jax.debug.print("Rollout complete: last_obs shape = {}", last_obs.shape)

        # 2) GAE advantage
        _, last_val = network.apply(train_state.params, last_obs)
        jax.debug.print("After rollout: last_val shape = {}", last_val.shape)
        ...
        def ppo_update(train_state, traj_batch, advantages, returns, rng):
            ...
            jax.debug.print("After flattening: traj_obs shape = {}", traj_flat.obs.shape)
            jax.debug.print("Advantage flat shape = {}", adv_flat.shape)
            jax.debug.print("Return flat shape = {}", ret_flat.shape)
            ...
            def update_minibatch(train_state, minibatch):
                ...
                grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
                (loss_val, aux_vals), grads = grad_fn(train_state.params, traj_mb, adv_mb, ret_mb)
                jax.debug.print("Minibatch loss: {}", loss_val)
                train_state = train_state.apply_gradients(grads=grads)
                return train_state, loss_val
...
def evaluate_policy_deterministic(train_state, network, env, env_params, rng, max_steps=10000):
    obs, state = env.reset(rng, env_params)
    jax.debug.print("Evaluation: Initial obs shape = {}", obs.shape)
    done = False
    total_reward = 0.0
    steps = 0
    while (not done) and (steps < max_steps):
...
def run_ppo_training(config):
    ...
    # Build the network and dummy observation
    obs_shape = env.observation_space(env_params).shape
    action_dim = env.action_space(env_params).n
    if "MiniGrid" in config["ENV_NAME"]:
        network = MiniGridCNNActorCritic(action_dim=action_dim)
    ...
    jax.debug.print("Dummy observation shape: {}", dummy_obs.shape)
    ...
    reset_rngs = jax.random.split(reset_rng, config["NUM_ENVS"])
    obsv, env_state = jax.vmap(env.reset, in_axes=(0, None))(reset_rngs, env_params)
    jax.debug.print("Initial observation shape: {}", obsv.shape)

    # Build rollout + update function
    rollout_and_update_fn = make_rollout_and_update_fn(env, env_params, network, config)
    jax.debug.print("Rollout and update function built.")
    ...
    for update_i in range(config["NUM_UPDATES"]):
        jax.debug.print("Update {} starting, global_env_steps = {}", update_i, global_env_steps)
        train_state, env_state, obsv, rng, traj_batch = rollout_and_update_fn(
            train_state, env_state, obsv, rng
        )
        global_env_steps += steps_per_update

        # Process episode returns
        info_dict = traj_batch.info
        returned_ep_ret = np.array(info_dict["returned_episode_returns"])
        returned_ep = np.array(info_dict["returned_episode"])
        ended_idx = np.where(returned_ep > 0)
        ...
        jax.debug.print("Update {} complete.", update_i)

        if max(eval_ret if eval_ret is not None else -1e9, median_train_return) >= config["OPTIMAL_REWARD"]:
            print(f"Optimal reward reached after {global_env_steps} env steps (metric: {max(eval_ret if eval_ret else -1e9, median_train_return):.3f}).")
            break
