...
with added JAX profiler instrumentation.
"""
...
import optax
import wandb
import jax.profiler  # Import the profiler
...
def make_rollout_and_update_fn(env, env_params, network, config):
    ...
    def rollout_and_update(train_state, env_state, last_obs, rng):
        ...
        def ppo_update(train_state, traj_batch, advantages, returns, rng):
            ...
            # Run multiple epochs (UPDATE_EPOCHS)
...
def run_ppo_training(config):
    ...
    # 5) Build rollout + update function
    ...
    print(f"Started JAX profiler trace in {trace_dir}")
    ...
    for update_i in range(config["NUM_UPDATES"]):
        ...
        if max(eval_ret if eval_ret else -1e9, median_train_return) >= config["OPTIMAL_REWARD"]:
            print(
                f"Optimal reward reached after {global_env_steps} env steps "
                f"(metric: {max(eval_ret, median_train_return):.3f})."
            )
            break
