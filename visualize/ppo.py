...
- At the end of training, the script saves the network’s train state as a checkpoint.
...
def run_ppo_training(config):
    ...
    checkpoints.save_checkpoint(ckpt_dir=ckpt_dir, target=train_state, step=global_env_steps, overwrite=True)
    print(f"Checkpoint saved at step {global_env_steps}.")
